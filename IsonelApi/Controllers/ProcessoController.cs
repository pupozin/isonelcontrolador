...
namespace IsonelApi.Controllers
{
    ...
    public class ProcessoController : ControllerBase
    {
        ...
        [HttpGet("insights")]
        public async Task<IActionResult> ObterInsightsProcessos([FromQuery] int ano, [FromQuery] int mes)
        {
            if (ano < 2000 || ano > 2100 || mes < 1 || mes > 12)
            {
                return BadRequest("Parametros de ano/mes invalidos.");
            }

            var resposta = new InsightsProcessosDto();
            ...
            {
                var resumo = new InsightsResumoDto();
                resumo.ProcessosIniciados = reader.IsDBNull(reader.GetOrdinal("ProcessosIniciados"))
                    ? 0
                    : reader.GetInt32(reader.GetOrdinal("ProcessosIniciados"));
                resumo.ProcessosFinalizados = reader.IsDBNull(reader.GetOrdinal("ProcessosFinalizados"))
                    ? 0
                    : reader.GetInt32(reader.GetOrdinal("ProcessosFinalizados"));
                resumo.DuracaoMediaProcessoMin = reader.IsDBNull(reader.GetOrdinal("DuracaoMediaProcessoMin"))
                    ? null
                    : reader.GetDecimal(reader.GetOrdinal("DuracaoMediaProcessoMin"));
                resumo.DuracaoMedianaProcessoMin = reader.IsDBNull(reader.GetOrdinal("DuracaoMedianaProcessoMin"))
                    ? null
                    : reader.GetDecimal(reader.GetOrdinal("DuracaoMedianaProcessoMin"));
                resumo.DuracaoMediaEtapaMin = reader.IsDBNull(reader.GetOrdinal("DuracaoMediaEtapaMin"))
                    ? null
            ...
            if (await reader.NextResultAsync())
            {
                ...
                {
                    resposta.EtapasPorTipo.Add(new InsightsEtapaTipoDto
                    {
            ...
            if (await reader.NextResultAsync())
            {
                while (await reader.ReadAsync())
                {
                    resposta.ProcessosMaisLongos.Add(new InsightsProcessoDestaqueDto
                    {
                        ProcessoId = reader.GetInt32(reader.GetOrdinal("ProcessoId")),
            ...
            if (await reader.NextResultAsync())
            {
                while (await reader.ReadAsync())
                {
                    resposta.ProcessosPorDia.Add(new InsightsProcessosPorDiaDto
                    {
            ...
            if (await reader.NextResultAsync())
            {
                while (await reader.ReadAsync())
                {
                    resposta.Responsaveis.Add(new InsightsResponsavelDto
                    {
